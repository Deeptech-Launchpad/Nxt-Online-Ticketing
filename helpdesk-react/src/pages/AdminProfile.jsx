...
export default function AdminProfile() {
  ...
  const [form, setForm] = useState({
    ...
    designation: currentUser?.designation || 'Solo IT Support',
    dept:        currentUser?.dept        || 'IT Operations',
    division:    currentUser?.division    || 'AltiusNxt Headquarters',
  });
  const [saving, setSaving] = useState(false);

  // Re-sync form whenever currentUser changes (e.g. after login)
  useEffect(() => {
  ...
  const handleSave = async () => {
    ...
    setSaving(false);
    if (result.ok) showToast('Admin settings saved', 'success');
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 6, justifyContent: 'center', marginBottom: 8, flexWrap: 'wrap' }}>
            ...
            <span style={{
              ...
              {form.dept || 'IT Operations'}
            </span>
          </div>

          <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 18 }}>
            {form.division || 'AltiusNxt Headquarters'}
          </div>
